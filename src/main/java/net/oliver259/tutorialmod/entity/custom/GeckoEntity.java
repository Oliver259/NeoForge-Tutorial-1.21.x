...
//    }
    ...
    // Spawn baby colour based on parents
    @Override
    public @Nullable AgeableMob getBreedOffspring(ServerLevel level, AgeableMob otherParent) {
        GeckoEntity baby = ModEntities.GECKO.get().create(level);
        if(baby != null && otherParent instanceof GeckoEntity otherGecko) {
            GeckoVariant parentVariant = this.getVariant();
            GeckoVariant otherVariant = otherGecko.getVariant();

            GeckoVariant chosenVariant = level.getRandom().nextBoolean() ? parentVariant : otherVariant;
